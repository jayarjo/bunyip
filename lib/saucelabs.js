	...
	*/
	...
	util.extend(this, {
		...
		, isTunnelierActive: function() {
			var self = this;
			return SauceLabs.prototype.isTunnelierActive.call(this)
				.then(function(output) {
					if (fs.existsSync(path.join(self.tunnelierPath, _tunnelierReadyFile))) {
						return Q.resolve(output);
					} else {
						return Q.reject(util.format("SauceLabs: Process is running but ready file (%s) is not present.", _tunnelierReadyFile));
					}
				})
				;
		}
		...
		, killTunnelier: function() {
			...
			return SauceLabs.prototype.killTunnelier.call(this)
				.done(function() {
					var readyFile = fs.existsSync(path.join(self.tunnelierPath, _tunnelierReadyFile));
