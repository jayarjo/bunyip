...
function SauceLabs(options) {
	...
	*/
	...
	function _exposeToAgents() {
		var tunnelierPath = path.resolve('tools/Sauce-Connect.jar');	

		return _getTunellier(tunnelierPath)
			.then(function() {
				console.log("Launching tunnelier...");
				return exec(util.format("java -jar %s %s %s", tunnelierPath, username, accessKey));
			});
	}


	function _connect(uri, agentId) {
		...
		return this.resolve(agentId)
			.then(function(agent) {
				...
				.then(function(sessionId) {
					...
					return conn.get(url.format(uri));
				});
			});
	}
	...
	util.extend(this, {
		...
		, connect: function(hubUri, agents) {
			var self = this;
			...
			return isLocal(hubUri.domain)
				.then(function(result) {
					return result ? _exposeToAgents.call(self, hubUri) : hubUri;
				})
				.then(function(hubUri) {
					var 
					  queue = []
					, i = agents.length
					;
					while (i--) {
						queue.push(_connect.call(self, hubUri, agents[i]));
					}
					return Q.all(queue);
				});	
		}
