...
program
	...
	.option('-x, --killall', 'kill all browserstack workers')
	.action(function(cmd) {
		cmd.isSet = true; // without this it is hard to tell whether command was supplied
	}) 
	;
...
program
	.command('tunnel')
	.option('-l, --list', 'list established tunnels')
	.option('-k, --kill <id>', 'kill tunnel process')
	.option('-x, --killall', 'kill all tunnel processes')
	.action(function(cmd) {
		cmd.isSet = true;
	})
	;
