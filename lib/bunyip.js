		...
		if (bs) {
			tester.requiredBrowsers(function(browsers) {
				if (browsers && browsers.length) {
					// check if those browsers are not yet queued
					bs.getBrowsers(function(error, currBrowsers) {
						if (!error && currBrowsers.length) {
							browsers = browsers.filter(function(ua) {
								var notRunning = true, uaId = util.uaIdGen(ua);

								util.each(currBrowsers, function(currUa) {
									if (uaId === util.uaIdGen(currUa)) {
