...
function expose(urls, toPort, cb) {
	...
	function makeItPublic() {
		// ... expose it
		var url, tunnel = new Tunnel(config.tunnel, config.wait);
...
function connectToSwarm(files, swarmUrl) {
	...
	tester.on('agentComplete', function(client, results) {
		/* client sample 
		{ id: '61',
		  uaID: 'Firefox|13',
		...
		*/
		...
		error(good, "Agent completed:", client.uaID.replace(/\|/, ' '));
	});
...
function connectToYeti(files, hubUrl) {
	...
	tester.on('agentComplete', function(agent, details) {
		/* details sample
			{ Utils: 
			   { name: 'Utils',
			     passed: 58,
