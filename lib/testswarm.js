...
function TestSwarm(config, agents) {
		...
		swarmState(function(result) {
			...
			if (currState) {
				util.each(UAs, function(ua, uaId) {
					var diff, agentName = UAs[uaId].data.displaytitle;

					if (!currState[uaId]) {
						if (config.verbose) {
							console.log("Agent connected: " + uaId);
						}
						self.emit('agentConnect', uaId, agentName, UAs[uaId].stats);
						return true;
					}
					
					diff = UAs[uaId].stats.onlineClients - currState[uaId].stats.onlineClients;
					if (diff > 0) {
	...
	function pollResults(jobId) {
		...
		jobStatus(jobId, function(job) {
			var run = job.runs[0], tasks = [];	

			util.each(run.uaRuns, function(uaRun, uaId) {
				var runId, report;

 				uaId = uaId.toLowerCase(); // we can unify id, since we do not need original version anymore
				...
				if (completedAgents[jobId] && completedAgents[jobId][uaId]) {
					return true;
				}

				if (!completedAgents.hasOwnProperty(jobId)) {
					completedAgents[jobId] = {};
				}
				...
				completedAgents[jobId][uaId] = true; // flag as completed

				tasks.push(function(cb) {
					runResult(uaRun.runId, function(result) {
						util.extend(uaRun, result.resultInfo);
						
						completedAgents[jobId][uaId] = uaRun; // now store full result
						self.emit('agentComplete', result.client, uaRun);
						cb(null);
					});
				});
			});
			...
			if (tasks.length) {
				async.parallel(tasks, function(error, results) {
					var completed = checkCompleted(jobId);
					if (completed) {
						deleteJob(jobId, function() {
							self.emit('complete', jobId, completed);
						});
					} else {
						setTimeout(function() {
							pollResults(jobId);
