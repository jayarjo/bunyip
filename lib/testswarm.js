...
function TestSwarm(config, agents) {
	...
	function requiredBrowsers(cb) {
		...
		cb(agents.filter(function(ua) {
			var uaId = util.uaIdGen(ua);
			return !connectedAgents[uaId] && !completedAgents[uaId];
		}));
	}
	...
	function agentCompleted(uaId, job) {
		...
		util.each(job.runs, function(run) {
			...
			if (!completedRuns[uaRun.runResultsID]) {
	...
	function jobCompleted(job) {
		...
		util.each(job.runs, function(run) {
			...
			util.each(agents, function(ua) {
				var uaId = util.uaIdGen(ua), uaRun = run.uaRuns[uaId];

				// here we use that runId that we've saved previously
				if (!uaRun || !completedAgents[uaId]) { 
					return (completed = false);
				} else {
					uaRun = completedRuns[uaRun.runResultsID];
		...
		jobStatus(jobId, function(job) {
			...
			util.each(job.runs, function(run) {
				...
				util.each(run.uaRuns, function(uaRun, uaId) {
					...
					completedRuns[uaRun.runResultsID] = true; // mark as completed


					tasks.push(function(cb) {
						runResult(uaRun.runResultsID, function(result) {
							completedRuns[uaRun.runResultsID] = util.extend(uaRun, result.resultInfo); // extend with aditional result info
